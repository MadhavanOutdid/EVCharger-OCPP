...
app.get('/start', (req, res) => {const parsedUrl = url.parse(req.url, true);
    const queryParams = parsedUrl.query;
        ...
        .then(existingDocument => {
            if (existingDocument) {
                ...
                collection.updateOne(
                    ...
                    { $set: {status: ChargerStatus.status, timestamp: ChargerStatus.timestamp, errorCode: ChargerStatus.errorCode} }
                )
                    .then(result => {
                        if (result) {
